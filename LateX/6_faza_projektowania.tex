...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{Obrazy/devops/architecture.png}
    \caption{Architektura aplikacji selfimproment.ai}
    \label{fig:my_label}
\end{figure}
...
\item {\bf Kind} - Jest narzędziem służącym do uruchamiania lokalnych klastrów Kubernetes za pomocą kontenerów Dockera. Idealnie nadaje się do testowania Kubernetes w izolowanym środowisku na pojedynczym komputerze, co jest przydatne w rozwoju oprogramowania i testowaniu.
...
W ramach zastosowanych praktyk bezpieczeństwa wykorzystano pipeline do automatyzacji procesów testowania i wdrażania, co zapewnia ciągłość i spójność dostarczania oprogramowania. Użycie narzędzia Terraform umożliwiło zarządzanie infrastrukturą jako kodem, zwiększając kontrolę i przejrzystość konfiguracji systemu. Dodatkowo, zaimplementowano rozwiązania takie jak Azure KeyVault do bezpiecznego przechowywania i zarządzania secretami, co znacząco podnosi poziom zabezpieczeń. Wszelkie poufne dane, takie jak hasła czy klucze API, są przechowywane jako secrety i zmienne środowiskowe, co ogranicza ryzyko ich wykorzystania przez nieuprawnione osoby i zapewnia lepszą kontrolę dostępu do wrażliwych zasobów.